...
var ttlcache = core.NewSieve[netip.Addr, int](context.TODO(), "d.ttl", desync_cache_ttl)
...
func tracert(d protect.RDialer, ipp netip.AddrPort, basePort int) (*net.UDPConn, int, error) {
	...
	if err != nil {
		return uc, udpFD, log.EE("desync: err announcing udp: %v", err)
	}
	if uc == nil {
		return uc, udpFD, log.EE("desync: nil udp conn")
	}
	...
	if err != nil {
		return uc, udpFD, log.EE("desync: %s => %s sockopt(recverr) err: %v", laddr(uc), ipp, err)
	}
...
func desyncWithTraceroute(d protect.RDialer, local, remote netip.AddrPort) (*overwriteSplitter, error) {
	...
	for i := 0; i < desync_max_ttl-1 && measureTTL; i += desync_delta_ttl {
		...
		if err != nil {
			log.VV("desync: recvmsg %v, err: %v", remote, err)
			break
		}
...
func (s *overwriteSplitter) Write(b []byte) (n int, err error) {
	...
	if err != nil {
		return 0, log.EE("desync: %s => %s get sock fd failed; %v", laddr, raddr, err)
	}
	...
	if err != nil {
		return 0, log.EE("desync: %s => %s setsockopt(ttl) err: %v", laddr, raddr, err)
	}
	var offset int64 = 0
	n1, err := unix.Sendfile(sockFD, fileFD, &offset, len(s.payload))
	if err != nil {
		return n1, log.EE("desync: %s => %s sendfile() %d err: %v", laddr, raddr, n1, err)
	}
	...
	if err != nil {
		return n1, log.EE("desync: %s => %s setsockopt(ttl) err: %v", laddr, raddr, err)
	}
...
func (s *overwriteSplitter) ReadFrom(reader io.Reader) (bytes int64, err error) {
	...
	bytes += b
	log.V("desync: readfrom: done; sz: %d %s=>%s; dur: %s, wait: %s; err: %v",

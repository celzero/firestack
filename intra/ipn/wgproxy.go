...
func (w *wgproxy) Refresh() (err error) {
	...
	logei(err)("proxy: wg: %s: refresh done; len(dns): %d, len(peer): %d; viaOK? %t, didWait? %t / reset? %t / status: %s; err? %v",
		w.tag(), n, nn, viaOK, didWait, resetDevice, pxstatus(w.Status()), err)
	return
}

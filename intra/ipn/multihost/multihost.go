...
func (m *MHMap) Get(hostOrIpport string) (mh *MH, err error) {
	m.RLock()
	defer m.RUnlock()
	...
	logeif(mh == nil)("multihost: %s map: get: for %s; mh? %t, by ip? %t; parse-err: %v",
		m.k, host, mh != nil, err == nil, err)
...
func (m *MHMap) Len() (n int64) {
	...
	m.RLock()
	defer m.RUnlock()
	for _, mh := range m.byIpp {
		n += int64(mh.Len())
	}
	for _, mh := range m.byName {
		n += int64(mh.Len())
	}
	return
}
...
func (m *MHMap) String() string {
	...
	m.RLock()
	mhs := make(map[*MH]struct{}, max(len(m.byIpp), len(m.byName)))
	for _, h := range m.byIpp {
		if h == nil {
			continue
		}
		mhs[h] = struct{}{}

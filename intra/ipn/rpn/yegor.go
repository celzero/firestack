...
func convertToRegionalWgConfs(id *WsWgCreds, reservation *WsWgConnectData, list []WsServerList, test bool, port string) ([]*RegionalWgConf, error) {
...
keyagain:
	...
	if !useExistingCreds {
		...
		if err != nil {
			return nil, nil, nil, log.EE("ws: wgconfs: gen key #%d err: %v", runkey, err)
		}
	} else {
		var err error
		...
		if err != nil {
			return nil, nil, nil, log.EE("ws: wgconfs: existing key #%d err: %v", runkey, err)
		}
	}
	pub := priv.Mult()
	pubkeybase64 := pub.Base64()

	log.I("ws: wgconfs: gen creds: pubkey: %s, existing key #%d? %t; force? %t; ops: %v", trunc8(pubkeybase64), runkey, useExistingCreds, forceInit, &ops)
...
initagain:
	...
	details := fmt.Sprintf("pub: %s, keyed#%d? %t; usingExisting#%d? %t; forceinit? %t; ops: %v",
		trunc8(pubkeybase64), runkey, keyNeedsInit, runinit, useExistingCreds, force == "1", &ops)
	...
	if keyNeedsInit {
		...
		if err != nil {
			return nil, nil, nil, log.EE("ws: wgconfs: %s req err: %v", details, err)
		}
		...
		if err != nil || initres == nil {
			return nil, nil, nil, log.EE("ws: wgconfs: %s res err (nil? %t / tok? %s): %v", details, initres == nil, tokst, err)
		}
		...
		creds = &d.Config
		if d.Success != 1 {
			return nil, nil, nil, log.EE("ws: wgconfs: %s success != 1; debug: %v", details, d.Debug)
		}
		if len(d.Config.PrivateKey) <= 0 { // private key is generated locally (by the client)
			...
			if len(d.Config.PublicKey) > 0 && d.Config.PublicKey != pubkeybase64 { // registered public key must match the local one
				return nil, nil, nil, log.EE("ws: wgconfs: pubkey mismatch; expected %s, got %s",
	...
	if creds == nil || len(creds.PublicKey) <= 0 || len(creds.PrivateKey) <= 0 {
		return nil, nil, nil, log.EE("ws: wgconfs: missing pub/priv creds %s", details)
	}
	...
	*/
	...
	regconfs, err := convertToRegionalWgConfs(creds, &wgConnect.Data, servers, test, port)

	if err != nil || len(regconfs) <= 0 {
		return nil, nil, nil, log.EE("ws: wgconfs: (test? %t) no regions found %s; %v", test, details, err)
	}
	...
	permaCreds, permaErr := getOrCreatePermaCreds(h, sess, ent, existingPermaCreds)
	if permaErr != nil {
...
// getOrCreatePermaCreds checks whether existingPermaCreds is still registered on the server
// (via WgConfigs/list_keys); if so it is reused unchanged. Otherwise WgConfigs/permanent is
// called to generate a fresh permanent config which is then returned.
func getOrCreatePermaCreds(h *http.Client, sess *WsSession, ent *WsEntitlement, existingPermaCreds *WsWgPermanentConfig) (*WsWgPermanentConfig, error) {
	if sess == nil || ent == nil {
		return nil, errWsNoSession
	}
	bearer := sess.SessionToken
	...
	var kerr error
	// verify existing key is still active.
	if existingPermaCreds != nil && len(existingPermaCreds.PublicKey) > 0 {
		tokst := "sess-" + tokenState(bearer)
		for range 2 {
			...
			if kerr != nil || keys == nil {
				// errored = true
				log.E("ws: conf: perma: list keys err (nil? %t / tok? %s): %v", keys == nil, tokst, kerr)
				wsBriefPauseBeforeRetry()
				continue
			}
			for _, k := range keys.Data.PubKeys {
				if k == existingPermaCreds.PublicKey {
					log.I("ws: conf: perma: existing key %s active (%s); reusing", trunc8(existingPermaCreds.PublicKey), tokst)
					return existingPermaCreds, nil
				}
			} // TODO: no match, error out instead?
			break
		}
	}

	if kerr != nil { // TODO: on kerr, creating creds?
		return nil, log.EE("ws: conf: perma: failed verification; err: %v", kerr)
	}

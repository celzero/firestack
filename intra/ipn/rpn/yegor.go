...
/*
	    ...
	    {
	            "PrivateKey": "stdbase64",
	            "PublicKey": "3lXKBJMJ/Rv1BbUhojSkywk/GKnRV0JzDA1xtn50eFA=",
	            "PresharedKey": "stdbase64",
	            "AllowedIPs": "0.0.0.0/0",
	            "AllowedIPsV6": "::/0",
	            "HashedCIDR": [
	            ...
	            "Address": "100.127.91.163/32",
	            "AddressV6": "fd54:4::aaff:5ba3:ee48:e72b/128",
	            "DNS": "10.255.255.2"
	    }
	}
*/
type WsWgCreds struct {
	...
	// Address and DNS (and their v6 counterparts) are populated only by the
	// /permanent endpoint; empty for /init responses.
	...
	AddressV6 string `json:"AddressV6,omitempty"` // CIDR notation, e.g. "fd54:4::aaff:5ba3:ee48:e72b/128"; empty when unassigned
	DNSV6     string `json:"DNSV6,omitempty"`     // IPv6 address, e.g. "fd54::1"; empty unless the server supplies one
}
...
type WsWgInterface struct {
	...
	AddressV6 string `json:"AddressV6,omitempty"` // cidr notation, e.g. "fd54:4::aaff:5ba3:ee48:e72b/128"; empty when unassigned
	DNSV6     string `json:"DNSV6,omitempty"`     // ip address; empty unless the server supplies one
}
...
func convertToRegionalWgConfs(id *WsWgCreds, list []WsServerList, test bool, port string) ([]*RegionalWgConf, error) {
	...
	for _, server := range list {
		...
		for _, group := range sorted {
			...
			dnsaddr6 := id.DNSV6 // empty unless the server supplies one
			// Use any IPv4 permutation of AllowedIPs. The API only sends a hint.
			// Route v6 into the tunnel only when the creds carry a v6 address;
			// otherwise v6 stays firewalled (v4-only allowed ips).
			allowed := []string{gw4}
			hasaddr6 := len(id.AddressV6) > 0
			if hasaddr6 {
				allowed = append(allowed, gw6)
			}
...
connectagain:
	...
	*/
	...
	if len(creds.AddressV6) <= 0 {
		creds.AddressV6 = wgConnect.Data.Config.AddressV6
	}
	if len(creds.DNSV6) <= 0 {
		creds.DNSV6 = wgConnect.Data.Config.DNSV6
	}

	log.I("ws: wgconfs: got connect data; %s; config addr: %s / v6: %s, dns: %s / v6: %s; perma? %t",
		details, wgConnect.Data.Config.Address, wgConnect.Data.Config.AddressV6,
		wgConnect.Data.Config.DNS, wgConnect.Data.Config.DNSV6, perma)

...
const minFragBlks = (60 + MinTCPHeaderSize) / 8 // max IPv4 header + basic TCP header in fragment blocks (8 bytes each)
...
func (q *Parsed) decode4(b []byte) {
	...
	q.Src = withIP(q.Src, netip.AddrFrom4([4]byte{b[12], b[13], b[14], b[15]}))
	q.Dst = withIP(q.Dst, netip.AddrFrom4([4]byte{b[16], b[17], b[18], b[19]}))
	...
	if fragOfs == 0 {
		...
		case TCP:
			...
			q.TCPFlags = TCPFlag(sub[TCPFlagsOffset])

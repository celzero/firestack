...
func TestRadixSearch(t *testing.T) {
	...
	v0 := r.Get(goog) // goog if r.Set(goog, "goog") is uncommented; empty otherwise
	v1 := r.Get(wildgoog)
	v2 := r.Get(mailgoog)
	v3 := r.Get(dnsgoog) // empty
	v4 := r.Get(pgoog)   // empty regardless of r.Set(goog, "goog")

	t.Log("v0?: ", v0, "\tv1: ", v1, "\tv2: ", v2, "\tv3?: ", v3, "\tv4?: ", v4)
	...
	w4 := r.GetAny(pgoog)    // empty regardless of r.Set(goog, "goog")

	t.Log("w0: ", w0, "\tw1: ", w1, "\tw2: ", w2, "\tw3: ", w3, "\tw4: ", w4)
}
